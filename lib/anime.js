...
const author = 'Kyto Kin'
function neonime.search(query, page = 1) {
...
function neonime.ongoing(page = 1) {
...
function otakudesu.search(query) {
		return new Promise((resolve) => {
...
function otakudesu.getData(url) {
		return new Promise((resolve) => {
...
function otakudesu.download(url) {
		return new Promise((resolve) => {
			axios.get(url).then(({data}) => {
				const $ = cheerio.load(data)
				const result = {}
